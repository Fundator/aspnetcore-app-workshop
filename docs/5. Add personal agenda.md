...
In this section we'll add features that track attendees who have signed in via Azure AD allow them to create a personal agenda.

## Add BackEnd attendee and FrontEnd user association
1. Add a `Welcome.cshtml` razor page and `Welcome.cshtml.cs` page model in the `Pages` folder in the FrontEnd project.
...
1. Add a folder called `Filters` in the FrontEnd project.
1. Add a new attribute `SkipWelcomeAttribute.cs` to allow certain pages or action methods to be skipped from enforcing redirection to the Welcome page:
    ...
    ```
1. Add a new class called `RequireLoginFilter.cs` that redirects to the Welcome page if the user is authenticated but not associated with an attendee:
...
1. Change the logic in `OnGetAsync` to get session using the new virtual method we just added:
    ...
    ```
1. We also need a list of user sessions in the `Index.cshtml.cs` model. Add a property `UserSessions`:
   ```csharp
   public List<int> UserSessions { get; set; }
   ```
   We populate this list from the `OnGetAsync` method in `Index.cshtml.cs`:
