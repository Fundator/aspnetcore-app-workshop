...
1. The data annotations and EF references can't be resolved to a missing NuGet package. Install the packages `System.ComponentModel.Annotations` and `Microsoft.EntityFrameworkCore.Sqlite` to the ConferenceDTO project. When the package restore completes, you should see that your dependencies are now resolved.
